...
func setup_sliders() -> void:
	configure_slider(acceleration_slider, 1.0, 40.0, 0.5)
	configure_slider(max_speed_slider, 1.0, 30.0, 0.5)
	configure_slider(steering_slider, 0.1, 3.0, 0.05)
	configure_slider(drag_slider, 0.0, 20.0, 0.5)

	configure_slider(jump_velocity_slider, 2.0, 15.0, 0.25)
	configure_slider(charged_jump_velocity_slider, 4.0, 20.0, 0.25)
	configure_slider(max_charge_time_slider, 0.1, 2.0, 0.05)
	configure_slider(air_control_slider, 0.0, 1.0, 0.05)

	configure_slider(bounce_retention_slider, 0.0, 1.0, 0.05)
	configure_slider(bounce_input_window_slider, 0.05, 0.5, 0.01)

	configure_slider(boost_acceleration_multiplier_slider, 1.0, 3.0, 0.05)
	configure_slider(boost_max_speed_multiplier_slider, 1.0, 2.5, 0.05)
	configure_slider(boost_release_deceleration_slider, 0.0, 15.0, 0.25)
...
func setup_trick_sliders() -> void:
	configure_slider(spin_acceleration_slider, 0.0, 2000.0, 10.0)
	configure_slider(max_spin_speed_slider, 90.0, 1440.0, 10.0)
	configure_slider(spin_drag_slider, 0.0, 720.0, 10.0)

	configure_slider(tuck_spin_multiplier_slider, 1.0, 3.0, 0.05)
	configure_slider(tuck_transition_speed_slider, 0.5, 20.0, 0.25)

	configure_slider(dive_angle_slider, 0.0, 180.0, 5.0)
	configure_slider(dive_speed_slider, 0.5, 20.0, 0.25)

	configure_slider(landing_realign_speed_slider, 0.5, 20.0, 0.25)
	configure_slider(landing_spin_deceleration_slider, 0.0, 3000.0, 25.0)

	spin_acceleration_slider.value_changed.connect(_on_spin_acceleration_changed)
	max_spin_speed_slider.value_changed.connect(_on_max_spin_speed_changed)
	spin_drag_slider.value_changed.connect(_on_spin_drag_changed)

	tuck_spin_multiplier_slider.value_changed.connect(_on_tuck_spin_multiplier_changed)
	tuck_transition_speed_slider.value_changed.connect(_on_tuck_transition_speed_changed)

	dive_angle_slider.value_changed.connect(_on_dive_angle_changed)
	dive_speed_slider.value_changed.connect(_on_dive_speed_changed)

	landing_realign_speed_slider.value_changed.connect(_on_landing_realign_speed_changed)
...
func update_parameter_labels() -> void:
	...
	charged_jump_velocity_value.text = (
		"Charged Jump Velocity: %.2f" % ball.charged_jump_velocity
	)
	...
	bounce_input_window_value.text = (
		"Bounce Input Window: %.2f s" % ball.bounce_input_window
	)
...
func update_player_parameter_labels() -> void:
	...
	spin_drag_value.text = "Spin Drag: %.0f°/s²" % player.spin_drag
	...
	dive_angle_value.text = "Dive Angle: %.0f°" % player.dive_angle
	dive_speed_value.text = "Dive Speed: %.2f" % player.dive_speed
	...
	landing_spin_deceleration_value.text = (
		"Landing Spin Deceleration: %.0f°/s²" % player.landing_spin_deceleration
	)

#endregion


#region Debug Readouts
...
func update_jump_debug() -> void:
	...
	if ball.is_charging_jump:
		if charge_ratio >= 1.0:
			set_status(jump_status_label, "Jump: FULL", STATUS_FULL_COLOR)
		else:
			set_status(jump_status_label, "Jump: CHARGING", STATUS_WARNING_COLOR)
	elif ball.is_on_floor():
		set_status(jump_status_label, "Jump: READY", STATUS_READY_COLOR)
	else:
		set_status(jump_status_label, "Jump: AIRBORNE", STATUS_ACTIVE_COLOR)
...
func update_bounce_debug() -> void:
	...
	if ball.bounce_input_timer > 0.0:
		...
		set_status(bounce_status_label, "Bounce: READY", STATUS_READY_COLOR)
	else:
		set_status(bounce_status_label, "Bounce: WAITING", STATUS_INACTIVE_COLOR)
...
func update_boost_debug() -> void:
	...
	if ball.is_boosting:
		set_status(boost_status_label, "Boost: ACTIVE", STATUS_ACTIVE_COLOR)
	elif ball.is_on_floor():
...
func update_player_debug() -> void:
	...
	current_jump_height_label.text = (
		"Current Height: %.2f m" % player.get_current_jump_height()
	)
	last_jump_height_label.text = (
		"Last Jump: %.2f m" % player.get_last_jump_height()
	)
	best_jump_height_label.text = (
		"Best Jump: %.2f m" % player.get_best_jump_height()
	)
	catch_distance_label.text = (
		"Catch Distance: %.2f m" % player.get_catch_distance()
	)

	if absf(rpm) > 1.0:
		spin_rpm_label.add_theme_color_override("font_color", STATUS_ACTIVE_COLOR)
	else:
		spin_rpm_label.remove_theme_color_override("font_color")

	if tuck_amount >= 0.99:
		tuck_label.add_theme_color_override("font_color", STATUS_FULL_COLOR)
	elif tuck_amount > 0.01:
		tuck_label.add_theme_color_override("font_color", STATUS_WARNING_COLOR)
	else:
		tuck_label.remove_theme_color_override("font_color")

	if player.is_airborne():
		set_status(airborne_label, "Airborne: YES", STATUS_ACTIVE_COLOR)
	else:
		set_status(airborne_label, "Airborne: NO", STATUS_INACTIVE_COLOR)

	if player.is_diving():
		set_status(dive_status_label, "Dive: ACTIVE", STATUS_ACTIVE_COLOR)
	else:
		set_status(dive_status_label, "Dive: OFF", STATUS_INACTIVE_COLOR)

#endregion


#region Parameter Cues
...
func setup_parameter_cues() -> void:
	...
	register_parameter_cue(charged_jump_velocity_slider, charged_jump_velocity_value)
	register_parameter_cue(max_charge_time_slider, max_charge_time_value)
	register_parameter_cue(air_control_slider, air_control_value)

	register_parameter_cue(bounce_retention_slider, bounce_retention_value)
	register_parameter_cue(bounce_input_window_slider, bounce_input_window_value)
	...
	register_parameter_cue(spin_acceleration_slider, spin_acceleration_value)
	register_parameter_cue(max_spin_speed_slider, max_spin_speed_value)
	register_parameter_cue(spin_drag_slider, spin_drag_value)
	register_parameter_cue(tuck_spin_multiplier_slider, tuck_spin_multiplier_value)
	register_parameter_cue(tuck_transition_speed_slider, tuck_transition_speed_value)
	register_parameter_cue(dive_angle_slider, dive_angle_value)
	register_parameter_cue(dive_speed_slider, dive_speed_value)
	register_parameter_cue(landing_realign_speed_slider, landing_realign_speed_value)
	register_parameter_cue(
...
func register_parameter_cue(slider: HSlider, label: Label) -> void:
	original_slider_values[slider] = slider.value
	slider_labels[slider] = label
	slider.value_changed.connect(_on_parameter_cue_changed.bind(slider))


func _on_parameter_cue_changed(_value: float, slider: HSlider) -> void:
...
func update_parameter_cue(slider: HSlider) -> void:
	...
	if is_equal_approx(slider.value, original_value):
		...
		label.add_theme_color_override("font_color", MODIFIED_COLOR)
